...
class MessytableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_L_rgb = np.array(Image.open(self.img_L[idx]).convert(mode='RGB').resize((960,540), resample=Image.BILINEAR))
        img_R_rgb = np.array(Image.open(self.img_R[idx]).convert(mode='RGB').resize((960,540), resample=Image.BILINEAR))
